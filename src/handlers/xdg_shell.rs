...
pub fn add_mapped_toplevel_pre_commit_hook(toplevel: &ToplevelSurface) -> HookId {
    add_pre_commit_hook::<State, _>(toplevel.wl_surface(), move |state, _dh, surface| {
        ...
        let mut has_buffer_blocker = false;
        if let Some(acquire_point) = acquire_point {
            if let Ok((blocker, source)) = acquire_point.generate_blocker() {
                if let Some(client) = surface.client() {
                    let mut transaction_for_syncobj = transaction_for_dmabuf.clone();
                    let res = state
                        .niri
                        .event_loop
                        .insert_source(source, move |_, _, state| {
                            // This surface is now ready for the transaction.
                            drop(transaction_for_syncobj.take());
                    ...
                    if res.is_ok() {
                        transaction_for_dmabuf = None;
                        add_blocker(surface, blocker);
                        trace!("added DRM syncobj acquire-point blocker");
                        has_buffer_blocker = true;
                    }
                }
            }
        }

        if !has_buffer_blocker {
